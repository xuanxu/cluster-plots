...
# docker build -t cluster_plots .
# docker run -d -p 80:80 -e RAILS_MASTER_KEY=<value from config/master.key> --name cluster_plots cluster_plots
...
RUN apt-get update -qq && \
    apt-get install --no-install-recommends -y curl wget libjemalloc2 libvips sqlite3 libyaml-dev && \
...
RUN groupadd --system --gid 1000 rails && \
    ...
    chown -R rails:rails db log storage tmp libext
USER 1000:1000
